...
public class ScanDeviceTool {
    ...
    public List<String> scan() {
        ...
        mExecutor = new ThreadPoolExecutor(CORE_POOL_SIZE, MAX_IMUM_POOL_SIZE,
                5000, TimeUnit.MILLISECONDS, new ArrayBlockingQueue<Runnable>(
                CORE_POOL_SIZE));
        ...
        for (int i = 1; i < 255; i++) {// 创建256个线程分别去ping
            ...
            Runnable run = new Runnable() {
                ...
                public void run() {
                    ...
                    String ping = ScanDeviceTool.this.mPing + currentIp;
                    ...
                    try {
                        ...
                        if (result == 0) {
                            ...
                            // 手机是拿不到的
//                            NbtAddress nbtAddress = NbtAddress.getByName("192.168.221.110");
//                            String firstCalledName = nbtAddress.firstCalledName();
//                            String nextCalledName = nbtAddress.nextCalledName();
//                            Log.d(TAG, "firstCalledName-->>" + firstCalledName);
//                            Log.d(TAG, "nextCalledName-->>" + nextCalledName);
...
//                            Log.d(TAG, "扫描失败");
            ...
            try {
                if (mExecutor.isTerminated()) {// 扫描结束,开始验证
                    ...
                    if (mScanListener != null) {
                        mScanListener.onScanFinish(mIpList, mDevAddress);
                    }
                    destory();
                    return mIpList;
                }
    ...
    public interface ScanListener {
        void onScanFinish(List<String> ipList, String devAddress);
    }
